...
// Cloud: hand-drawn cloud with rain drops
...
// Dedicated: server rack with drive bays
...
// Shared: a folder with an upload arrow
...
// Website: browser window with lightning bolt
...
/* ══════════════════════════════════════════════════════
   VPS — Live resource-monitor dashboard
   Canvas sparkline + animated CPU/RAM/DISK/NET gauges
   ══════════════════════════════════════════════════════ */
function VpsAnimation() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.strokeStyle = "rgba(46,18,74,0.06)";
      ctx.lineWidth = 1;
      for (let y = 0; y <= 4; y++) {
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 20px" }}>
          ...
          {gauges.map(g => (
            <div key={g.label} style={{ borderRadius: 12, border: "1px solid rgba(46,18,74,0.08)", background: "rgba(250,247,253,0.9)", padding: "10px 12px" }}>
              ...
              <div style={{ height: 5, borderRadius: 99, background: "rgba(46,18,74,0.07)", overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${g.value}%`, borderRadius: 99, background: g.color, boxShadow: `0 0 8px ${g.color}60`, transition: "width 0.8s cubic-bezier(0.16,1,0.3,1)" }} />
              </div>
            </div>
          ))}
        </div>
...
/* ══════════════════════════════════════════════════════
   Cloud — World map with animated PoP nodes + data packets
   ══════════════════════════════════════════════════════ */
function CloudAnimation() {
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px 16px" }}>
      <div style={{ width: "100%", maxWidth: 340 }}>
        ...
        <div style={{ borderRadius: 16, overflow: "hidden", border: "1px solid rgba(46,18,74,0.1)", background: "linear-gradient(160deg,#FAF7FD,#F5F0FB)" }}>
          <canvas ref={canvasRef} width={320} height={200} style={{ width: "100%", height: 200, display: "block" }} />
        </div>
...
/* ══════════════════════════════════════════════════════
   Dedicated — Physical server rack with real-time telemetry
   ══════════════════════════════════════════════════════ */
        ...
        <div style={{ borderRadius: 14, border: "2px solid rgba(46,18,74,0.2)", background: "rgba(250,247,253,0.98)", overflow: "hidden" }}>
          ...
          <div style={{ padding: "10px 12px", display: "flex", flexDirection: "column", gap: 6 }}>
            {units.map((u, i) => {
              ...
              return (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 8, border: `1px solid ${isActive ? `${tc}50` : "rgba(46,18,74,0.08)"}`, background: isActive ? `${tc}10` : "rgba(46,18,74,0.02)", transition: "all 0.5s cubic-bezier(0.16,1,0.3,1)" }}>
...
/* ══════════════════════════════════════════════════════
   Shared — cPanel file-manager + one-click deploy flow
   ══════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════
   Website — LiteSpeed performance meter + request waterfall
   ══════════════════════════════════════════════════════ */
    ...
    <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px 16px" }}>
      <div style={{ width: "100%", maxWidth: 320, display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <div style={{ borderRadius: 12, border: "1px solid rgba(46,18,74,0.09)", background: "rgba(250,247,253,0.95)", overflow: "hidden" }}>
          ...
          <canvas ref={canvasRef} width={290} height={108} style={{ width: "100%", height: 108, display: "block" }} />
        </div>
  ...
  return (
    ...
    >
      ...
      <div ref={ref} style={{ position:"relative",maxWidth:1280,margin:"0 auto",padding:isMobile?"4rem 5% 5rem":"7rem 5%",zIndex:1 }}>
        ...
        {/* TABS */}
        <div style={{ display:"flex",flexWrap:"wrap" as const,gap:isMobile?8:10,marginBottom:isMobile?24:40 }}>
          {products.map((p, i) => {
            const isActive = active === i;
            return (
              <button key={p.id} onClick={() => switchTab(i)}
                className={isActive ? "" : "prod-tab"}
                style={{ position:"relative",padding:isMobile?"9px 14px":"11px 22px",borderRadius:12,fontSize:isMobile?"0.8rem":"0.875rem",fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:7,background:isActive?`linear-gradient(135deg,${P},${A})`:"#fff",border:isActive?`1px solid rgba(46,18,74,0.4)`:`1px solid rgba(46,18,74,0.12)`,color:isActive?"#fff":`rgba(46,18,74,0.6)`,boxShadow:isActive?`0 6px 24px rgba(46,18,74,0.25)`:"none",transition:"all 0.35s cubic-bezier(0.34,1.56,0.64,1)",transform:isActive?"translateY(-2px)":"translateY(0)",animation:tabsVisible?`prod-tabIn 0.8s ${i*0.12}s both cubic-bezier(0.34,1.56,0.64,1)`:"none",opacity:tabsVisible?undefined:0,overflow:"hidden" }}
              >
        ...
        {/* CARD */}
        <div style={{ display:"grid",gridTemplateColumns:isMobile||isTablet?"1fr":"1fr 1fr",borderRadius:isMobile?16:24,overflow:"hidden",border:`1px solid rgba(46,18,74,0.12)`,boxShadow:`0 24px 80px rgba(46,18,74,0.1),0 4px 20px rgba(46,18,74,0.06)`,animation:animating?"prod-panelOut 0.22s ease forwards":"none" }}>

          {/* Animation panel */}
          <div style={{ position:"relative",minHeight:isMobile?320:isTablet?380:460,background:`linear-gradient(160deg,#FAF7FD,#F5F0FB,#EDE8F8)`,borderRight:isMobile||isTablet?"none":`1px solid rgba(46,18,74,0.1)`,borderBottom:isMobile||isTablet?`1px solid rgba(46,18,74,0.1)`:"none",animation:cardVisible?`${isMobile||isTablet?"prod-cardUp":"prod-cardLeft"} 1.1s 0s both cubic-bezier(0.16,1,0.3,1)`:"none",opacity:cardVisible?undefined:0 }}>
          ...
          {/* Text panel */}
          <div style={{ padding:isMobile?"1.75rem 1.5rem":isTablet?"2rem 2.25rem":"2.5rem 2.75rem",background:"#fff",display:"flex",flexDirection:"column",justifyContent:"space-between",animation:cardVisible?`${isMobile||isTablet?"prod-cardUp":"prod-cardRight"} 1.1s 0.15s both cubic-bezier(0.16,1,0.3,1)`:animating?"none":"prod-tabSwitch 0.35s 0.1s both ease",opacity:cardVisible?undefined:animating?1:0 }}>
            <div>
